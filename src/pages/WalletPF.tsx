...
import { Plus, RefreshCw, User, Pencil, Trash2, CheckCircle, ArrowUpRight, ArrowDownRight, Search } from 'lucide-react';
...
export default function WalletPF() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // Filter for PF (Pessoa Física) - individuals typically don't have company names
      // We'll use a simple heuristic: names without LTDA, ME, EIRELI, S.A., etc.
      ...
      const filterPF = (items: Account[], field: 'supplier' | 'customer') => {
        return items.filter(item => {
          ...
          // Exclude companies by common Brazilian company suffixes
  ...
  const applyFilters = (items: Account[], filters: AccountFiltersState) => {
    return items.filter(item => {
      ...
      if (filters.dateFrom) {
        ...
        if (isBefore(dueDate, filters.dateFrom)) {
          return false;
        }
      }
      ...
      if (filters.dateTo) {
        const dueDate = parseISO(item.due_date);
        if (isAfter(dueDate, filters.dateTo)) {
          return false;
        }
      }
      ...
      if (searchTerm) {
        ...
        if (!matchesDescription && !matchesSupplier && !matchesCustomer) {
          return false;
        }
      }
  ...
  const filteredPayables = useMemo(() => 
    applyFilters(payables, payableFilters), 
    [payables, payableFilters, searchTerm]
  );
  
  const filteredReceivables = useMemo(() => 
    applyFilters(receivables, receivableFilters), 
    [receivables, receivableFilters, searchTerm]
  );
  ...
  return (
    <MainLayout>
      <PageHeader
        ...
        description="Contas relacionadas a pessoas físicas"
      >
      ...
      <div className="grid gap-3 mb-6 grid-cols-2 lg:grid-cols-4">
        ...
        <div className="bg-card rounded-xl p-4 border border-border shadow-sm col-span-2">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-muted-foreground">Saldo Previsto (PF)</span>
          </div>
      ...
      <Tabs defaultValue="payables" className="w-full">
        ...
        <TabsContent value="payables">
          <AccountFilters
            ...
            onFiltersChange={setPayableFilters}
            type="payable"
          />
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          {(item.status === 'pending' || item.status === 'overdue') && (
                            <Button size="sm" variant="outline" className="h-8 text-xs gap-1" onClick={() => handleOpenMarkAsPaidModal(item, 'payable')}>
        ...
        <TabsContent value="receivables">
          <AccountFilters
            ...
            onFiltersChange={setReceivableFilters}
            type="receivable"
          />
          <div className="flex justify-end mb-4">
                    ...
                    <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          {(item.status === 'pending' || item.status === 'overdue') && (
                            <Button size="sm" variant="outline" className="h-8 text-xs gap-1" onClick={() => handleOpenMarkAsPaidModal(item, 'receivable')}>
                              <CheckCircle className="w-3 h-3" />
                              Receber
                            </Button>
                          )}
